(()=>{
  ...
  const getSettings=()=>{const rows=window.NOKTENA_CATALOG_BOOTSTRAP?.rows||[];const row=rows.find(r=>r.product_key===KEY);const p=row?.payload||{};return{enabled:p.enabled===true,title:Object.prototype.hasOwnProperty.call(p,'title')?String(p.title??''):'Цена сентября',subtitle:Object.prototype.hasOwnProperty.call(p,'subtitle')?String(p.subtitle??''):'до 30 сентября'};};
