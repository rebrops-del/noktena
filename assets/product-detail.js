    ...
    root.innerHTML=`<article class="pd-product">
      ...
      <div class="pd-content-column">
        <section class="pd-info-card">
          ...
          ${colors.length?`<div class="pd-choice"><div class="pd-choice-head"><label>Цвет</label><b id="pdColorName">${esc(selectedColor)}</b></div><div class="pd-colors">${colors.map((c,i)=>`<button type="button" class="pd-color ${i===0?'is-active':''}" data-pd-color="${esc(c)}" style="--swatch:${swatchCss(c)}"><span class="pd-color-dot" aria-hidden="true"></span><span>${esc(c)}</span></button>`).join('')}</div></div>`:''}
