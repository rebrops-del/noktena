  ...
  function cardMarkup(product){
    ...
    return `<article class="f-card product-open-card" data-product-id="${esc(product.id)}" data-product-link="${esc(link)}" data-selected-color="${esc(colors[0]||'')}" data-selected-size="${esc(sizes[0]||'')}">
      ...
      <div class="f-card-body">
        ...
        ${colors.length?`<div class="f-option"><div class="f-option-head"><span>Цвет</span><b class="f-color-name">${esc(colors[0])}</b></div><div class="f-color-list">${colors.slice(0,8).map((c,i)=>`<button type="button" class="f-color ${i===0?'is-active':''}" data-card-color="${esc(c)}" title="${esc(c)}" aria-label="Цвет ${esc(c)}">${esc(c)}</button>`).join('')}${colors.length>8?`<span class="f-more-options">+${colors.length-8}</span>`:''}</div></div>`:''}
