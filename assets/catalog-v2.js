(() => {
  ...
  function sections(){
    ...
    return {hero:$('.hero'),delivery:$('#delivery'),about:$('#about'),guide:$('#guide'),catalog:$('#catalog'),hits:$('#homeHits'),furniture:$('#furnitureCatalog'),cta};
  }
  ...
  function applyView(view,{scroll=true}={}){
    ...
    const s=sections();
    show(s.hero,view==='home');show(s.delivery,view==='home');show(s.about,view==='home');show(s.guide,view==='home');show(s.hits,view==='home');
    show(s.catalog,view==='mattresses');show(s.furniture,view==='beds'||view==='sofas');show(s.cta,true);
  ...
  function chooseSpecs(product){
    ...
    const out=[];
    for(const p of preferred){const e=entries.find(([k])=>k.toLowerCase()===p.toLowerCase());if(e&&!out.some(x=>x[0]===e[0]))out.push(e);if(out.length>=2)break;}
    for(const e of entries){if(out.length>=2)break;if(!out.some(x=>x[0]===e[0]))out.push(e);}
    return out;
  }
  ...
  function cardMarkup(product){
    const imgs=[...new Set((product.images||[]).filter(Boolean))];
    ...
    const link=detailUrl(product);
    state.gallery.set(product.id,0);
    return `<article class="f-card product-open-card" data-product-id="${esc(product.id)}" data-product-link="${esc(link)}">
      ...
      <div class="f-card-body">
        <div class="f-card-tags"><span class="f-card-tag">${category}</span>${subtype}</div>
        ...
        <div class="f-card-bottom">
          <div class="f-price-row"><div class="f-price">${rub(product.price)}</div><div class="f-price-note">Наличие и вариант<br>уточняйте при заказе</div></div>
          <a class="product-more-link" href="${esc(link)}">Подробнее о товаре <span aria-hidden="true">→</span></a>
  ...
  function setGallery(id,delta){
    ...
    const imgs=[...new Set((product.images||[]).filter(Boolean))];if(imgs.length<2)return;
    ...
    const img=$('img',g);if(!img)return;
    const preload=new Image();
    preload.onload=()=>{
      state.gallery.set(id,next);
      img.style.opacity='.25';
      requestAnimationFrame(()=>{img.src=imgs[next];img.style.opacity='1'});
    ...
    preload.onerror=()=>{state.gallery.set(id,next);};
    preload.src=imgs[next];
  }
  ...
  function filtered(view){
    ...
    items.sort((a,b)=>{if(sort==='price-desc')return (b.price||-1)-(a.price||-1);if(sort==='name')return String(a.title).localeCompare(String(b.title),'ru');return (a.price??Number.MAX_SAFE_INTEGER)-(b.price??Number.MAX_SAFE_INTEGER);});
    return items;
  }

  function paginationMarkup(view,total,page){
    const pages=Math.max(1,Math.ceil(total/PAGE_SIZE));if(pages<=1)return '';
    const nums=[];const from=Math.max(1,page-2),to=Math.min(pages,page+2);
    if(from>1)nums.push(1);if(from>2)nums.push('…');for(let i=from;i<=to;i++)nums.push(i);if(to<pages-1)nums.push('…');if(to<pages)nums.push(pages);
    return `<div class="f-pagination" aria-label="Страницы каталога"><button class="f-page" data-page-view="${view}" data-page="${page-1}" ${page<=1?'disabled':''}>←</button>${nums.map(n=>n==='…'?'<span class="f-page" style="display:grid;place-items:center;border:0;box-shadow:none;cursor:default">…</span>':`<button class="f-page ${n===page?'is-active':''}" data-page-view="${view}" data-page="${n}">${n}</button>`).join('')}<button class="f-page" data-page-view="${view}" data-page="${page+1}" ${page>=pages?'disabled':''}>→</button></div>`;
  }

  function renderFurniture(view){
    const mount=$('#furnitureGrid'),count=$('#furnitureCount'),title=$('#furnitureTitle'),desc=$('#furnitureDesc'),subtype=$('#fSubtype');if(!mount)return;
    if(title)title.textContent=view==='beds'?'Кровати':'Диваны';
    if(desc)desc.textContent=view==='beds'?'Выберите модель и откройте карточку товара — внутри все фотографии и характеристики.':'Прямые и угловые диваны. Откройте карточку товара, чтобы посмотреть все фотографии и характеристики.';
    ...
    const items=filtered(view);const pages=Math.max(1,Math.ceil(items.length/PAGE_SIZE));state.page[view]=Math.min(Math.max(1,state.page[view]),pages);const page=state.page[view];const slice=items.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);
  ...
  function renderHomeHits(){
    ...
    if(!state.loaded){mount.innerHTML='<div class="f-loading">Загружаем подборку…</div>';return;}
  ...
  async function loadFurniture(){
    try{const r=await fetch(`data/furniture.json?v=${Date.now()}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);const data=await r.json();state.data.beds=Array.isArray(data.beds)?data.beds:[];state.data.sofas=Array.isArray(data.sofas)?data.sofas:[];state.loaded=true;}
    catch(e){console.error('Furniture catalog load failed',e);state.loaded=true;}
