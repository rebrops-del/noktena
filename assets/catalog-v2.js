(() => {
  ...
  function cardMarkup(product){
    ...
    return `<article class="f-card product-open-card" data-product-id="${esc(product.id)}" data-product-link="${esc(link)}" data-selected-color="${esc(colors[0]||'')}" data-selected-size="${esc(sizes[0]||'')}">
      ...
      <div class="f-card-body">
        ...
        ${colors.length?`<div class="f-option"><div class="f-option-head"><span>Цвет</span><b class="f-color-name">${esc(colors[0])}</b></div><div class="f-color-list">${colors.slice(0,8).map((c,i)=>`<button type="button" class="f-color ${i===0?'is-active':''}" data-card-color="${esc(c)}" title="${esc(c)}" aria-label="Цвет ${esc(c)}" style="--swatch:${swatchCss(c)}"></button>`).join('')}${colors.length>8?`<span class="f-more-options">+${colors.length-8}</span>`:''}</div></div>`:''}
  ...
  function renderFurniture(view){const mount=$('#furnitureGrid'),count=$('#furnitureCount'),title=$('#furnitureTitle'),desc=$('#furnitureDesc'),subtype=$('#fSubtype');if(!mount)return;if(title)title.textContent=view==='beds'?'Кровати для продуманной спальни':'Диваны для комфортного отдыха';if(desc)desc.textContent=view==='beds'?'Выберите модель, цвет и спальное место. В полной карточке — фотографии, характеристики и доступные варианты.':'Прямые и угловые модели с выбором цвета и доступных вариантов. Для каждого цвета показываем соответствующую фотографию, когда она предоставлена в каталоге.';if(subtype){subtype.style.display=view==='sofas'?'block':'none';if(view!=='sofas'){subtype.value='';state.subtype='';}}if(!state.loaded){mount.innerHTML='<div class="f-loading">Загружаем коллекцию…</div>';return;}const items=filtered(view);const pages=Math.max(1,Math.ceil(items.length/PAGE_SIZE));state.page[view]=Math.min(Math.max(1,state.page[view]),pages);const page=state.page[view];const slice=items.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE);if(count)count.textContent=`${items.length} моделей · страница ${page} из ${pages}`;mount.innerHTML=slice.length?slice.map(cardMarkup).join(''):'<div class="f-empty">По выбранным параметрам модели не найдены.</div>';const pg=$('#furniturePagination');if(pg)pg.innerHTML=paginationMarkup(view,items.length,page);}
