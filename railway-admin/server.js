...
function liftLabel(method,floor){if(method==='cargo')return 'Грузовой лифт';if(method==='stairs')return `По лестнице${floor?`, этаж ${floor}`:''}`;return 'Без подъёма';}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(incoming.pathname==='/health')return sendJson(res,200,{ok:true,service:'noktena-admin-proxy',mail_configured:mailReady()});
  ...
  try{
    ...
    if(req.method==='POST'&&incoming.searchParams.get('action')==='create-order'&&body){try{orderRequest=JSON.parse(body.toString('utf8'))}catch{}}
    ...
    if(contentType.includes('application/json')){
      try{
        const data=rewriteValue(req,JSON.parse(raw.toString('utf8')));
        if(upstream.ok&&orderRequest&&data?.order){data.email_notification=await notifyOrderByEmail(orderRequest,data.order);}
