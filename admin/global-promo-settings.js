(()=>{
  ...
  function currentSettings(b){const row=(b.rows||[]).find(r=>r.product_key===KEY);const p=row?.payload||{};return{enabled:p.enabled===true,title:String(p.title??''),subtitle:String(p.subtitle??'')};}
