(()=>{
  ...
  function ensureStyles(){
    ...
    const style=document.createElement('style');style.id='ordersStyles';style.textContent=`
      .admin-tabs{display:flex;align-items:center;gap:6px;width:max-content;max-width:100%;margin:0 0 18px;padding:5px;border:1px solid #dce8e1;border-radius:14px;background:#eef4f1}.admin-tab{position:relative;appearance:none;border:0;border-radius:10px;padding:10px 17px;background:transparent;color:#52675f;font:inherit;font-weight:900;cursor:pointer;white-space:nowrap}.admin-tab:hover{background:#f8fbf9;color:#173e32}.admin-tab.is-active{background:#fff;color:#173e32;box-shadow:0 3px 10px rgba(23,62,50,.08)}.orders-badge{position:absolute;right:-7px;top:-8px;display:grid;place-items:center;min-width:22px;height:22px;padding:0 6px;border-radius:999px;background:#d92d3b;color:#fff;font-size:11px;font-weight:900;box-shadow:0 0 0 3px #f5f8f6}.orders-badge.hide{display:none!important}
      .admin-pane.hide{display:none!important}.orders-pane-head{display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:16px}.orders-pane-head h1{margin:0;color:var(--ink)}.orders-pane-panel{background:#fff;border:1px solid var(--line);border-radius:18px;padding:16px}.orders-toolbar{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 0 14px}.orders-summary{font-size:13px;color:var(--muted)}
      ...
      @media(max-width:620px){.admin-tabs{width:100%}.admin-tab{flex:1}.orders-pane-head{align-items:center}.orders-row{grid-template-columns:1fr auto;gap:6px 10px}.orders-row>*{text-align:left}.orders-row .orders-date,.orders-row .orders-phone{display:block}.orders-arrow{grid-column:2;grid-row:1/5;align-self:center}.orders-detail-grid{grid-template-columns:1fr}.orders-detail-title{font-size:20px}.orders-pane-panel{padding:12px}}
  ...
  function ensure(){
    ...
    const catalog=document.createElement('section');catalog.id='adminCatalogPane';catalog.className='admin-pane';
    while(main.firstChild)catalog.appendChild(main.firstChild);
    const tabs=document.createElement('div');tabs.id='adminTabs';tabs.className='admin-tabs';tabs.innerHTML=`
      <button id="catalogTab" class="admin-tab is-active" type="button">Каталог</button>
      <button id="ordersTab" class="admin-tab" type="button">Заказы <span id="ordersBadge" class="orders-badge hide">0</span></button>`;
  ...
  function showTab(name){
    ...
    $('#adminCatalogPane')?.classList.toggle('hide',orders);
    $('#adminOrdersPane')?.classList.toggle('hide',!orders);
    $('#catalogTab')?.classList.toggle('is-active',!orders);
    $('#ordersTab')?.classList.toggle('is-active',orders);
  }

  function updateBadge(rows=ordersCache){
    const count=(rows||[]).filter(o=>o.status==='new').length,badge=$('#ordersBadge'),btn=$('#ordersTab');
    if(!badge)return;
  ...
  async function open(){ensure();showTab('orders');backToList();await load(true)}
  async function poll(){if(pollBusy||!sess()?.access_token)return;pollBusy=true;try{await fetchOrders();if(!$('#adminOrdersPane')?.classList.contains('hide')){if(currentOrderId){const current=ordersCache.find(o=>o.id===currentOrderId);if(current)renderDetail(current);else backToList()}else renderList(ordersCache)}}catch{}finally{pollBusy=false}}

  document.addEventListener('click',async e=>{
    const del=e.target.closest('[data-order-delete]');
    if(del){
      const o=ordersCache.find(x=>x.id===del.dataset.orderDelete);if(!o)return;
      if(!confirm(`Удалить заказ № ${o.order_no}?\n\nЭто действие нельзя отменить.`))return;
      del.disabled=true;del.textContent='Удаляем…';
      try{
        await api('delete-order',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({id:o.id})});
        ordersCache=ordersCache.filter(x=>x.id!==o.id);updateBadge();backToList();renderList(ordersCache);
        if(typeof toast==='function')toast(`Заказ № ${o.order_no} удалён`);
      }catch(err){del.disabled=false;del.textContent='Удалить заказ';if(typeof toast==='function')toast(err.message,true)}
      return;
    }
    const row=e.target.closest('[data-order-open]');if(!row)return;const o=ordersCache.find(x=>x.id===row.dataset.orderOpen);if(o)renderDetail(o);
  });
